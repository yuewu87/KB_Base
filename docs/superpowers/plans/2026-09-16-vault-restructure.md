...
跑完 Task 12 后，本计划完成。代码侧应满足：

- 测试全绿，数量比计划开始时少（删了 26 个 + 新增若干）
- `docs/01_架构.md` 第十三节那张表里的 1–7、9、10 全部落地
- **8（定时整理）和 11（可移植打包）没做**——前者「暂时不管」，后者排在最后

vault 侧：`计算机/` 下有真实内容，分类长出来了，`_索引/` 有索引页与整理日志。
